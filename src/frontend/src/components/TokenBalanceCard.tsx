...
export default function TokenBalanceCard() {
  ...
  const { data: callerPrincipal, isLoading: principalLoading } =
    useGetCallerPrincipal();
  ...
  // Validation state
  const [icpValidationError, setIcpValidationError] = useState<string | null>(
    null,
  );
  ...
  const principalId =
    callerPrincipal?.toString() || identity?.getPrincipal().toString() || "";

  // Derive Account ID from Principal
  const accountId =
    principalId && callerPrincipal ? principalToAccountId(callerPrincipal) : "";
  ...
  const validateRecipient = (
    recipient: string,
  ): { valid: boolean; type: "account" | "invalid" } => {
  ...
  const handleSendICPClick = () => {
    ...
    console.log(
      "[TokenBalanceCard] Opening confirmation dialog for ICP transaction",
    );
  ...
  const handleSendPuffinClick = () => {
    ...
    console.log(
      "[TokenBalanceCard] Opening confirmation dialog for Puffin transaction",
    );
    setPendingTransaction({
      type: "PUFFIN",
  ...
  const handleConfirmTransaction = async () => {
    ...
    console.log(
      "[TokenBalanceCard] Starting transaction confirmation:",
      pendingTransaction,
    );
    ...
    try {
      if (pendingTransaction.type === "ICP") {
        const amountE8s = BigInt(
          Math.floor(Number.parseFloat(pendingTransaction.amount) * 100000000),
        );

        console.log("[TokenBalanceCard] Sending ICP transaction...");
        const result = await sendICP.mutateAsync({
        ...
        const amountTokens = BigInt(
          Math.floor(Number.parseFloat(pendingTransaction.amount) * 100000000),
        );

        console.log("[TokenBalanceCard] Sending Puffin transaction...");
        const result = await sendPuffin.mutateAsync({
          to: pendingTransaction.recipient,
          amount: amountTokens,
        });
      ...
      console.log(
        "[TokenBalanceCard] Closing confirmation dialog due to error",
      );
      setShowConfirmDialog(false);
      setPendingTransaction(null);
      ...
      if (error.message?.includes("Bonding Phase")) {
        toast.error("Puffin tokens not yet available", {
          description:
            "Token transfers will be enabled after the bonding phase",
          duration: 5000,
        });
        ...
        toast.error("Transaction timeout", {
          description:
            "The transaction took too long. Please check your connection and try again.",
          duration: 6000,
        });
                ...
                <div className="space-y-2.5 text-xs text-muted-foreground leading-relaxed">
                  <div className="flex items-start gap-2">
                    ...
                    <div>
                      ...
                      <p>
                        Use when transferring ICP to or from exchanges like{" "}
                        <span className="font-medium text-primary">
                          ICPSwap
                        </span>
                        , <span className="font-medium text-primary">NNS</span>,
                        or other platforms that require the hex account
                        identifier format.
                      </p>
                    </div>
                  </div>
                      ...
                      <div className="flex items-center gap-1.5">
                        ...
                        <Tooltip>
                          ...
                          <TooltipContent className="max-w-xs">
                            <p className="text-xs">
                              For deposits and withdrawals with exchanges
                            </p>
                          </TooltipContent>
                        </Tooltip>
                      </div>
                ...
                <div className="flex items-start gap-3">
                  ...
                  <div className="flex-1">
                    ...
                    <p className="text-xs text-muted-foreground leading-relaxed">
                      To receive ICP tokens, share your{" "}
                      <strong className="text-foreground">Account ID</strong>{" "}
                      with the sender or exchange.
                    </p>
                  </div>
                </div>
                  ...
                  <div className="space-y-3">
                    <div className="rounded-lg border border-primary/20 bg-white dark:bg-slate-900 p-3">
                      <div className="flex items-center justify-between gap-2 mb-2">
                        <div className="flex items-center gap-1.5">
                          <p className="text-xs font-semibold text-foreground">
                            Your Deposit Address (Account ID)
                          </p>
                          <Tooltip>
                            <TooltipTrigger asChild>
                              <Info className="h-3.5 w-3.5 text-muted-foreground cursor-help" />
                            ...
                            <TooltipContent className="max-w-xs">
                              <p className="text-xs">
                                Send ICP to this Account ID to deposit funds
                                into your wallet
                              </p>
                            </TooltipContent>
                          </Tooltip>
                        ...
                        <Button
                          ...
                          size="icon"
                          onClick={() =>
                            handleCopy(accountId, "Deposit Address")
                          }
                          className="h-7 w-7 flex-shrink-0"
                        ...
                        <div className="flex items-start gap-2">
                          ...
                          <div className="flex-1">
                            ...
                            <p className="text-xs text-amber-700 dark:text-amber-300 leading-relaxed">
                              Send ICP to your <strong>Account ID</strong> above
                              to deposit funds. Your balance will update
                      ...
                      <ul className="space-y-1.5 text-xs text-muted-foreground leading-relaxed">
                        <li className="flex items-start gap-2">
                          ...
                          <span>
                            ...
                            when receiving ICP from exchanges
                          </span>
                        </li>
                        <li className="flex items-start gap-2">
                          <span className="text-primary font-bold">•</span>
                          <span>
                            Your balance updates automatically after deposits
                            are confirmed
                          </span>
                        </li>
                      </ul>
              ...
              <div className="rounded-lg border-2 border-accent/20 bg-white dark:bg-slate-900 p-4 shadow-sm">
                ...
                <div className="space-y-2.5 pt-2 border-t border-accent/10">
                  ...
                  <Button
                    ...
                    disabled={
                      !puffinRecipient || !puffinAmount || isTransactionPending
                    }
