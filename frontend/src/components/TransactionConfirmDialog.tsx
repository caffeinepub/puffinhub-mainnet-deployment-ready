...
export default function TransactionConfirmDialog({
  ...
  useEffect(() => {
    console.log('[TransactionConfirmDialog] Modal state changed:', { open, isLoading, tokenType });
  ...
  const handleConfirm = async () => {
    console.log('[TransactionConfirmDialog] Confirm button clicked - starting transaction');
    try {
      await onConfirm();
      console.log('[TransactionConfirmDialog] onConfirm completed successfully');
    } catch (error) {
  ...
  const handleOpenChange = (newOpen: boolean) => {
    console.log('[TransactionConfirmDialog] Open state change requested:', newOpen);
        ...
        <div className="relative z-10 space-y-4 py-4">
          ...
          <div className="rounded-lg border-2 border-primary/20 bg-white dark:bg-slate-900 p-4 space-y-3 shadow-sm">
            <div className="flex justify-between items-start">
              <span className="text-sm font-medium text-slate-600 dark:text-slate-400">Token Type:</span>
              <span className="text-sm font-bold text-primary">{tokenType}</span>
            </div>
            <div className="flex justify-between items-start">
            ...
            {tokenType === 'ICP' && (
              <div className="flex justify-between items-start">
                <span className="text-sm font-medium text-slate-600 dark:text-slate-400">Transfer Fee:</span>
                <span className="text-sm font-medium text-accent">{transferFee}</span>
              </div>
            )}
          ...
          <div className="rounded-lg border border-accent/30 bg-accent/5 p-3">
            <p className="text-xs text-slate-700 dark:text-slate-300 leading-relaxed">
              <strong className="text-slate-900 dark:text-white">⚠️ Important:</strong> This transaction cannot be reversed. Please verify the recipient address and amount before confirming.
            </p>
          </div>
        </div>

        <AlertDialogFooter className="relative z-10">
          <AlertDialogCancel onClick={handleCancel} disabled={isLoading} className="border-primary/30">
            Cancel
          </AlertDialogCancel>
          <AlertDialogAction
