...
import { Trophy, Gift, Loader2, CheckCircle2, AlertCircle, Clock, FlaskConical } from 'lucide-react';
import { useUnifiedIdentity } from '../hooks/useUnifiedIdentity';
import { useGetUnclaimedRewards, useGetRewardHistory, useClaimRewards } from '../hooks/useQueries';
...
export default function RewardsPage() {
  ...
  const [claimResult, setClaimResult] = useState<{ blockIndex: bigint; amount: bigint } | null>(null);

  const { data: unclaimedRewards, isLoading: loadingRewards, refetch: refetchRewards } = useGetUnclaimedRewards();
  const { data: rewardHistory, isLoading: loadingHistory, refetch: refetchHistory } = useGetRewardHistory();
  ...
  const unclaimedICP = unclaimedRewards ? Number(unclaimedRewards) / 100000000 : 0;
            ...
            <CardContent className="pt-6">
              <div className="flex items-start gap-3">
                ...
                <div className="flex-1">
                  <h3 className="font-semibold text-blue-900 dark:text-blue-100 mb-1">Daily Reward Limits</h3>
                  <p className="text-sm text-blue-800 dark:text-blue-200">
                    To ensure fair distribution, there are daily limits on reward claims:
                  </p>
                  <ul className="mt-2 space-y-1 text-sm text-blue-800 dark:text-blue-200">
                    <li className="flex items-center gap-2">
                      <span className="h-1.5 w-1.5 rounded-full bg-blue-600 dark:bg-blue-400"></span>
                      <span><strong>Global limit:</strong> 50 total rewards per day</span>
                    </li>
                    <li className="flex items-center gap-2">
                      <span className="h-1.5 w-1.5 rounded-full bg-blue-600 dark:bg-blue-400"></span>
                      <span><strong>Personal limit:</strong> 5 rewards per player per day</span>
                    </li>
                  </ul>
                  <p className="mt-2 text-xs text-blue-700 dark:text-blue-300">
                    Limits reset daily at midnight UTC. If you reach your limit, try again tomorrow!
                  </p>
                </div>
              </div>
            </CardContent>
            ...
            <CardHeader>
              <div className="flex items-center gap-3">
                ...
                <div>
                  ...
                  <CardDescription>Your pending ICP rewards from Colony Wars victories</CardDescription>
                </div>
              </div>
            </CardHeader>
            <CardContent>
              {loadingRewards ? (
                ...
                <div className="space-y-6">
                  <div className="rounded-lg bg-gradient-to-br from-primary/10 to-accent/10 p-6 text-center">
                    <div className="mb-2 text-sm font-medium text-muted-foreground">Available to Claim</div>
                    <div className="mb-1 text-5xl font-bold text-primary">
                      {unclaimedICP.toFixed(4)}
                    </div>
                    <div className="text-lg font-medium text-muted-foreground">ICP</div>
                  </div>
                  ...
                  {hasUnclaimedRewards ? (
                    ...
                    <div className="rounded-lg border-2 border-dashed border-primary/30 bg-primary/5 p-6 text-center">
                      ...
                      <p className="text-sm font-medium text-muted-foreground">
                        No unclaimed rewards available. Win more Colony Wars battles to earn rewards!
                      </p>
                    </div>
                  )}
                  ...
                  <div className="rounded-lg bg-blue-50 p-4 dark:bg-blue-950/30">
                    <p className="text-sm text-blue-900 dark:text-blue-100">
                      <strong>How it works:</strong> Earn 0.001 ICP for each verified Colony Wars victory. 
                      Rewards are automatically tracked and can be claimed at any time, subject to daily limits.
                    </p>
                  </div>
                </div>
              )}
                  ...
                  {rewardHistory.map((claim, index) => (
                    ...
                    >
                      ...
                      <div className="text-right">
                        <div className="text-xs text-muted-foreground">Block Index</div>
                        <div className="font-mono text-sm">{claim.blockIndex.toString()}</div>
                      </div>
                    </div>
                  ))}
                </div>
        ...
        <DialogContent className="bg-white dark:bg-slate-900 sm:max-w-md">
          ...
          <div className="space-y-4 py-4">
            <div className="rounded-lg bg-gradient-to-br from-primary/10 to-accent/10 p-4 text-center">
              <div className="mb-1 text-sm font-medium text-muted-foreground">Amount to Claim</div>
            ...
            <div className="rounded-lg bg-amber-50 p-3 dark:bg-amber-950/30">
              <p className="text-xs text-amber-900 dark:text-amber-100">
                <strong>Note:</strong> Daily limits apply (5 rewards per player, 50 total per day). 
                If limits are reached, you can claim again tomorrow.
              </p>
            </div>
            <p className="text-sm text-muted-foreground">
              The rewards will be transferred to your wallet. A small ledger fee (0.0001 ICP) will be deducted from the treasury.
            </p>
          </div>
            ...
            <div className="space-y-4 py-4">
              <div className="rounded-lg bg-gradient-to-br from-green-50 to-green-100 p-4 dark:from-green-950/30 dark:to-green-900/30">
                <div className="mb-3 text-center">
                  <div className="mb-1 text-sm font-medium text-muted-foreground">Claimed Amount</div>
                ...
                <div className="space-y-2 text-sm">
                  <div className="flex justify-between">
                    ...
                    <span className="font-mono font-semibold">{claimResult.blockIndex.toString()}</span>
                  </div>
                </div>
              </div>
              <p className="text-center text-sm text-muted-foreground">
                The ICP has been transferred to your wallet. Check your balance to confirm!
              </p>
            </div>
          )}
          <DialogFooter>
