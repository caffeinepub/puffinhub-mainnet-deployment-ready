...
import { RotateCcw, ArrowLeft, Trophy, Coins, Heart, Zap, Pause, Play, FastForward, ChevronRight, Shuffle, Info } from 'lucide-react';
...
type TowerType = 'pebble' | 'fishCannon' | 'iceMage' | 'wavecaller' | 'lookout' | 'engineer';
type EnemyType = 'gull' | 'crab' | 'seal' | 'jelly' | 'krakenling' | 'stormLord';
...
const ENEMY_DATA: Record<EnemyType, {
  name: string;
  baseHealth: number;
  baseSpeed: number;
  reward: number;
...
const ROUTE_DATA: Record<RouteType, {
  name: string;
  path: Position[];
}> = {
  route1: {
...
export default function PuffinTowerDefenseGame({ onBack }: PuffinTowerDefenseGameProps) {
  ...
  const [selectedTowerType, setSelectedTowerType] = useState<TowerType | null>('pebble');
  ...
  const getWaveComposition = (waveNum: number): EnemyType[] => {
    ...
    for (let i = 0; i < baseCount; i++) {
      if (waveNum >= 8) {
        composition.push(Math.random() < 0.3 ? 'seal' : Math.random() < 0.5 ? 'crab' : 'gull');
      } else if (waveNum >= 5) {
  ...
  const isValidPlacement = (x: number, y: number): boolean => {
    ...
    const exists = towersRef.current.some(t => 
      Math.abs(t.x - x) < TILE_SIZE && Math.abs(t.y - y) < TILE_SIZE
    );
  ...
  const handleCanvasClick = (e: React.MouseEvent<HTMLCanvasElement>) => {
    ...
    const clickedTower = towersRef.current.find(t => 
      Math.abs(t.x - x) < 32 && Math.abs(t.y - y) < 32
    );
  ...
  const drawBackground = (ctx: CanvasRenderingContext2D) => {
    ...
    ctx.drawImage(nestImg, PATH[PATH.length - 1].x - 48, PATH[PATH.length - 1].y - 48, 96, 96);
  };
  ...
  const drawTopMenu = (ctx: CanvasRenderingContext2D) => {
    ...
    heartImg.src = '/assets/generated/clean-heart-lives-icon-transparent.dim_24x24.png';
  ...
  const drawPlacementPreview = (ctx: CanvasRenderingContext2D) => {
    ...
    ctx.fillStyle = isValid ? 'rgba(30, 136, 229, 0.3)' : 'rgba(244, 67, 54, 0.4)';
    ctx.fillRect(gridX * TILE_SIZE, gridY * TILE_SIZE, TILE_SIZE, TILE_SIZE);
    
    // Draw glow border
    ctx.strokeStyle = isValid ? 'rgba(30, 136, 229, 0.6)' : 'rgba(244, 67, 54, 0.8)';
    ctx.lineWidth = 3;
    ...
    // Draw range preview
    ctx.strokeStyle = isValid ? 'rgba(30, 136, 229, 0.3)' : 'rgba(244, 67, 54, 0.3)';
    ctx.lineWidth = 2;
    ctx.beginPath();
  ...
  const drawEnemy = (ctx: CanvasRenderingContext2D, enemy: Enemy) => {
    ...
    const img = new Image();
    img.src = data.image;
    ctx.drawImage(img, enemy.x - data.size / 2, enemy.y - data.size / 2, data.size, data.size);
    ...
    ctx.fillRect(enemy.x - barWidth / 2, enemy.y - data.size / 2 - 12, barWidth, barHeight);
    
    // Health fill
    ctx.fillStyle = healthPercent > 0.5 ? '#4CAF50' : healthPercent > 0.25 ? '#FFC107' : '#F44336';
    ctx.fillRect(enemy.x - barWidth / 2, enemy.y - data.size / 2 - 12, barWidth * healthPercent, barHeight);
    
    // Border
    ctx.strokeStyle = 'rgba(255, 255, 255, 0.8)';
    ctx.lineWidth = 1;
    ctx.strokeRect(enemy.x - barWidth / 2, enemy.y - data.size / 2 - 12, barWidth, barHeight);
  ...
  const gameLoop = (timestamp: number) => {
    ...
    const deltaTime = isPaused ? 0 : (timestamp - lastTimestampRef.current) * gameSpeed;
    lastTimestampRef.current = timestamp;
    ...
    if (waveInProgress && !isPaused) {
      ...
      if (spawnTimerRef.current >= 800 && enemiesSpawnedRef.current < waveComposition.length) {
      ...
      if (enemiesSpawnedRef.current >= waveComposition.length && enemiesRef.current.length === 0) {
        setWaveInProgress(false);
        setWave(prev => prev + 1);
      ...
      enemiesRef.current = enemiesRef.current.filter(enemy => {
        ...
        if (enemy.type === 'krakenling' && enemy.regenTimer !== undefined) {
          ...
          if (enemy.regenTimer >= 1000) {
            enemy.health = Math.min(enemy.health + enemy.maxHealth * 0.02, enemy.maxHealth);
            enemy.regenTimer = 0;
          }
        }
    ...
    towersRef.current.forEach(tower => {
      ...
      if (!isPaused && timestamp - tower.lastFired >= tower.fireRate / gameSpeed) {
        // Find target
        ...
        if (target) {
          const isSplash = tower.type === 'fishCannon' || tower.type === 'wavecaller';
      ...
      projectilesRef.current = projectilesRef.current.filter(projectile => {
        ...
        if (distance < moveSpeed) {
          ...
          if (projectile.splash) {
            ...
            const target = enemiesRef.current.find(e => e.id === projectile.targetId);
            if (target) {
  ...
  }, [isGameActive, wave, waveInProgress, selectedTower, selectedRoute, placementMode, gameSpeed, isPaused]);
      ...
      <div className="mx-auto max-w-7xl">
        <div className="mb-8 flex items-center gap-4">
          ...
          <div className="flex-1">
            <h1 className="text-3xl font-bold md:text-4xl">Puffin Tower Defense</h1>
                ...
                <div className="relative flex justify-center bg-muted p-4">
                  ...
                  {!isGameActive && !gameOver && (
                    <div className="absolute inset-0 flex items-center justify-center bg-black/40 rounded-lg">
                      <Button size="lg" onClick={() => setShowRouteSelect(true)} className="gap-2 shadow-2xl">
              ...
              {gameOver && showNameInput && (
                <Card>
                  <CardHeader>
                    ...
                    <CardDescription>You survived {wave - 1} waves! Final score: {score}</CardDescription>
                  </CardHeader>
                  <CardContent className="space-y-4">
                    ...
                    <div className="flex gap-2">
                      ...
                      <Button variant="outline" onClick={() => setShowRouteSelect(true)}>
                        Play Again
                      </Button>
                    </div>
                  </CardContent>
                ...
                <Card className="border-2 border-primary shadow-lg">
                  <CardHeader className="pb-3">
                    <CardTitle className="text-lg flex items-center gap-2">
                      <img src={TOWER_DATA[selectedTower.type].image} alt="" className="w-8 h-8" />
                      {TOWER_DATA[selectedTower.type].name}
                    </CardTitle>
                    <CardDescription>Level {selectedTower.level} • {TOWER_DATA[selectedTower.type].role}</CardDescription>
                  </CardHeader>
                  <CardContent className="space-y-3">
                    <div className="p-3 bg-accent/10 rounded-lg space-y-1 text-sm">
                      <div><strong>Attack Type:</strong> {TOWER_DATA[selectedTower.type].attackType}</div>
                      <div><strong>Special:</strong> {TOWER_DATA[selectedTower.type].specialAbility}</div>
                    </div>
                    <div className="grid grid-cols-2 gap-2 text-sm">
                      <div className="p-2 bg-accent/10 rounded">
                        <div className="text-xs text-muted-foreground">Damage</div>
                        <div className="font-bold text-accent">{selectedTower.damage}</div>
                      </div>
                      <div className="p-2 bg-primary/10 rounded">
                        <div className="text-xs text-muted-foreground">Range</div>
                        <div className="font-bold text-primary">{selectedTower.range}</div>
                      </div>
                      <div className="p-2 bg-green-500/10 rounded">
                        <div className="text-xs text-muted-foreground">Fire Rate</div>
                        <div className="font-bold text-green-600">{(1000 / selectedTower.fireRate).toFixed(1)}/s</div>
                      </div>
                      <div className="p-2 bg-yellow-500/10 rounded">
                        <div className="text-xs text-muted-foreground">Upgrade</div>
                        <div className="font-bold text-yellow-600">{selectedTower.upgradeCost} 🐟</div>
                      </div>
                    </div>
                    {selectedTower.specialEffect && (
                      <div className="p-2 bg-accent/10 rounded text-xs">
                        <strong>Current Upgrade:</strong> {selectedTower.specialEffect}
                      </div>
                    )}
                    {selectedTower.upgradeLevel < TOWER_DATA[selectedTower.type].upgrades.length ? (
                      <>
                        <div className="p-2 bg-primary/10 rounded text-xs">
                          <strong>Next Upgrade:</strong> {TOWER_DATA[selectedTower.type].upgrades[selectedTower.upgradeLevel]}
                        </div>
                        <Button 
                ...
                return (
                  ...
                  >
                    <div className="flex flex-col items-center gap-1">
                      ...
                      <div className="text-xs font-semibold text-center">{data.name}</div>
                      <div className="text-xs font-bold text-accent">{data.cost} 🐟</div>
                    </div>
                  </button>
                );
              })}
          ...
          <DialogHeader>
            ...
            <DialogDescription className="space-y-4 pt-4">
              <p className="text-base">
                Build puffin towers to defend your eggs from waves of enemies. Earn fish tokens to upgrade your defenses!
              </p>
              
              <div>
                <h3 className="font-bold text-base mb-2">Gameplay:</h3>
                <ul className="list-disc list-inside space-y-1 text-sm">
                  <li>Game starts at 0.5x speed - use speed controls to adjust gameplay pace</li>
                  <li>Select a tower type from the right sidebar to enter placement mode</li>
                  <li>Click on the map to place towers (avoid the path - invalid spots show in red)</li>
                  <li>Click on placed towers to view detailed stats and upgrade them</li>
                  <li>Click the "Start Wave" button overlay when ready to begin each wave</li>
              ...
              <div>
                ...
                <div className="space-y-2 text-sm">
                  <div><strong>Pebble-Launcher:</strong> Fast single-target attacks for consistent DPS</div>
                  <div><strong>Fish-Cannon:</strong> Splash damage projectiles hit multiple enemies</div>
                  <div><strong>Ice-Mage:</strong> Slows enemies with frost for crowd control</div>
                  <div><strong>Wavecaller:</strong> Large AOE wave attacks for heavy damage</div>
                  <div><strong>Lookout:</strong> Support tower that boosts nearby tower stats</div>
                  <div><strong>Engineer:</strong> Balanced mechanical projectiles with good range</div>
                </div>
              </div>
              ...
              <div>
                ...
                <div className="space-y-2 text-sm">
                  <div><strong>Greedy Gull:</strong> Fast but weak</div>
                  <div><strong>Sneaky Crab:</strong> Can burrow to avoid attacks</div>
                  <div><strong>Frost Seal:</strong> Tanky and freeze-resistant</div>
                  <div><strong>Jelly Swarm:</strong> Weak groups vulnerable to splash</div>
                  <div><strong>Krakenling:</strong> Mini-boss with regeneration</div>
                  <div><strong>Storm Gull Lord:</strong> Final boss with high health</div>
                </div>
              </div>
            </DialogDescription>
          </DialogHeader>
      ...
      <Dialog open={showRouteSelect} onOpenChange={setShowRouteSelect}>
        <DialogContent>
          <DialogHeader>
            ...
            <DialogDescription>Choose the path enemies will take across the island</DialogDescription>
          </DialogHeader>
            ...
            {(Object.keys(ROUTE_DATA) as RouteType[]).map((routeType) => {
              ...
              return (
                ...
                >
                  ...
                  <div className="text-sm text-muted-foreground">Strategic enemy pathing</div>
                </button>
              );
            })}
            <Button
