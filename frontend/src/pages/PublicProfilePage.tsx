...
import { useGetUserProfile, useGetPersonalStats } from '../hooks/useQueries';
...
export default function PublicProfilePage() {
  ...
  const { data: userProfile, isLoading: profileLoading } = useGetUserProfile(principal || Principal.anonymous());
  const { data: personalStats, isLoading: statsLoading } = useGetPersonalStats(principal || Principal.anonymous());
          ...
          <div className="grid gap-8 md:grid-cols-3">
              ...
              <CardContent className="space-y-4">
                <div className="flex flex-col items-center gap-4">
                  <Avatar className="h-32 w-32 border-4 border-primary/40 shadow-lg bg-white">
                    <AvatarImage src={getAvatarUrl()} alt={userProfile.displayName} />
            ...
            <Card className="md:col-span-2 border-2 border-primary/30 bg-gradient-to-br from-primary/10 to-background">
              <CardHeader>
                ...
                <CardDescription>Top scores across all Puffin games</CardDescription>
              </CardHeader>
              <CardContent>
                {personalStats ? (
                  <div className="grid gap-3 sm:grid-cols-2">
                    {Object.entries(personalStats).map(([game, score]) => {
                      ...
                      return (
                        ...
                        >
                          <span className="text-sm font-medium">{GAME_NAMES[game as keyof typeof GAME_NAMES]}</span>
                          <span className="text-lg font-bold text-primary">{Number(score)}</span>
                        </div>
                      );
                    })}
