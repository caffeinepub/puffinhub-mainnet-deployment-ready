...
export function useConnectedWallet() {
  ...
  const checkExistingConnections = async () => {
    try {
      ...
      console.error(
        "[useConnectedWallet] Error checking existing connections:",
        error,
      );
    }
  };
  ...
  const getICPBalance = useCallback(async (): Promise<bigint> => {
    ...
    try {
      ...
      const accountIdBytes = new Uint8Array(
        accountIdHex.match(/.{1,2}/g)!.map((byte) => Number.parseInt(byte, 16)),
      );
      ...
      console.log(
        "[useConnectedWallet] ICP balance in e8s:",
        balanceE8s.toString(),
      );
  ...
  const sendICP = useCallback(
    async (to: string, amountICP: number): Promise<{ blockIndex: bigint }> => {
      ...
      try {
        ...
        console.log("[useConnectedWallet] Sending ICP via Plug:", {
          to,
          amountICP,
        });
