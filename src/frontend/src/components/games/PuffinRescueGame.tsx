...
export default function PuffinRescueGame({ onBack }: PuffinRescueGameProps) {
  ...
  useEffect(() => {
    ...
    evilPuffinImg.src =
      "/assets/generated/evil-puffin-obstacle-animated-transparent.dim_48x48.png";
    ...
    for (let i = 0; i < config.predatorCount; i++) {
      ...
      while (attempts <= 30) {
        ...
        const distanceFromPlayer = Math.sqrt(
          (x - puffinRef.current.x) ** 2 + (y - puffinRef.current.y) ** 2,
        );
        attempts++;
        if (distanceFromPlayer >= MIN_DISTANCE_FROM_PLAYER) break;
      }
  ...
  const gameLoop = () => {
    ...
    predatorsRef.current.forEach((predator) => {
      ...
      if (predator.x < 0 || predator.x > CANVAS_WIDTH - predator.width) {
        ...
        predator.x = Math.max(
          0,
          Math.min(CANVAS_WIDTH - predator.width, predator.x),
        );
      }
      if (predator.y < 0 || predator.y > CANVAS_HEIGHT - predator.height) {
        ...
        predator.y = Math.max(
          0,
          Math.min(CANVAS_HEIGHT - predator.height, predator.y),
        );
      }
        ...
        <div className="mb-8 flex items-center gap-4">
          ...
          <div>
            ...
            <p className="mt-2 text-muted-foreground">
              Rescue puffin chicks and bring them to safety! Use arrow keys or
              WASD to move.
            </p>
          </div>
        </div>
          ...
          <DialogHeader>
            ...
            <DialogDescription className="space-y-4 pt-4">
              <p className="text-base">
                Use <strong>arrow keys</strong> to move. Rescue puffin chicks
                and bring them to safety while avoiding evil puffins!
              </p>
              <ul className="list-disc list-inside space-y-2 text-base">
                <li>
                  Use <strong>↑↓←→ (Arrow Keys)</strong> or{" "}
                  <strong>WASD</strong> to move in all directions
                </li>
                <li>
                  Touch <strong>golden chicks</strong> to rescue them
                </li>
                <li>
                  <strong>Avoid evil puffins</strong> with red eyes - they will
                  end your game!
                </li>
                <li>
                  Progress through <strong>{MAX_LEVEL} levels</strong> of
                  increasing difficulty
                </li>
                <li>
                  Each level has more chicks, more evil puffins, and faster
                  enemies
                </li>
