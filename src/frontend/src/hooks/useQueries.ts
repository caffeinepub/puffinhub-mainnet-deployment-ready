...
export function useGetCallerPrincipal() {
  ...
  return useQuery<Principal>({
    ...
    queryFn: async () => {
      ...
      try {
        console.log(
          "[useGetCallerPrincipal] Fetching caller principal from backend...",
        );
        const principal = await actor.getCallerPrincipal();
        console.log(
          "[useGetCallerPrincipal] Success - Principal:",
          principal.toString(),
        );
        return principal;
      } catch (error) {
        console.error(
          "[useGetCallerPrincipal] Error fetching caller principal:",
          error,
        );
        throw error;
      }
...
export function useGetICPBalance() {
  ...
  const query = useQuery<bigint>({
    ...
    queryFn: async () => {
      ...
      try {
        console.log(
          "[useGetICPBalance] Fetching live ICP balance for authenticated user...",
        );
        const balance = await actor.getICPBalance();
        console.log(
          "[useGetICPBalance] ✅ Success - Balance:",
          balance.toString(),
          "e8s",
        );
        console.log(
          "[useGetICPBalance] ✅ Balance in ICP:",
          (Number(balance) / 100000000).toFixed(8),
        );
        return balance;
      } catch (error: any) {
        console.error(
          "[useGetICPBalance] ❌ Error fetching ICP balance:",
          error,
        );
        ...
        if (error.message?.includes("TemporarilyUnavailable")) {
    ...
    retryDelay: (attemptIndex) => {
      ...
      console.log(
        `[useGetICPBalance] Retry attempt ${attemptIndex + 1} after ${delay}ms`,
      );
      return delay;
    },
  ...
  return useQuery<string[]>({
    ...
    queryFn: async () => {
      ...
      try {
        ...
        console.log(
          "[useGetRecentRecipients] Success - Found",
          recipients.length,
          "recipients",
        );
        return recipients;
        ...
        console.error(
          "[useGetRecentRecipients] Error fetching recent recipients:",
          error,
        );
        return [];
      }
...
export function detectRecipientFormat(
  recipient: string,
): "principal" | "account" | "invalid" {
...
export function validateICPTransaction(
  amountICP: string,
  ...
  if (totalRequired > balanceE8s) {
    ...
    const maxTransferableE8s =
      balanceE8s >= ICP_TRANSFER_FEE ? balanceE8s - ICP_TRANSFER_FEE : 0n;
    const maxTransferableICP = (Number(maxTransferableE8s) / 100000000).toFixed(
      8,
    );
...
export function useSendICP() {
  ...
  return useMutation({
    mutationFn: async ({ to, amount }: { to: string; amount: bigint }) => {
      console.log("[useSendICP] Mutation started:", {
        to,
      ...
      if (format === "invalid") {
        ...
        throw new Error(
          "Invalid recipient format: Must be either a Principal ID (with hyphens) or a 64-character hexadecimal Account ID",
        );
      }
      ...
      try {
        ...
        console.log(
          "[useSendICP] Transaction successful, block index:",
          blockIndex.toString(),
        );
        return {
          blockIndex,
        ...
        if (error.message?.includes("timeout")) {
          throw new Error(
            "Transaction timeout: Please check your connection and try again.",
          );
        }
        ...
        if (error.message?.includes("Invalid recipient")) {
          throw new Error(
            "Invalid recipient: Please check the address format.",
          );
        }
        if (error.message?.includes("Invalid Principal ID format")) {
          throw new Error(
            "Invalid Principal ID: Please verify the principal address.",
          );
        }
        if (error.message?.includes("Invalid Account ID format")) {
          throw new Error(
            "Invalid Account ID: Please verify the account identifier.",
          );
        }
        if (error.message?.includes("TemporarilyUnavailable")) {
          throw new Error(
            "Ledger temporarily unavailable: Please try again in a moment.",
          );
        }
        throw error;
      }
    ...
    onSuccess: (_data) => {
      console.log(
        "[useSendICP] Mutation onSuccess callback, invalidating queries",
      );
...
export function useSendPuffin() {
  ...
  return useMutation({
    mutationFn: async ({ to, amount }: { to: string; amount: bigint }) => {
      console.log("[useSendPuffin] Mutation started:", {
        to,
        amount: amount.toString(),
      });
      ...
      try {
        ...
        const blockIndex = await withTimeout(
          actor.sendPuffin({ toText: () => to } as any as Principal, amount),
          60000,
          "Transaction timeout: The ledger is taking too long to respond. Please try again.",
        );

        console.log(
          "[useSendPuffin] Transaction successful, block index:",
          blockIndex.toString(),
        );
        return {
          blockIndex,
        ...
        if (error.message?.includes("Bonding Phase")) {
          throw new Error(
            "Bonding Phase: Puffin token transfers not yet available",
          );
        }
        if (error.message?.includes("Insufficient")) {
          throw new Error(
            "Insufficient funds: You do not have enough Puffin tokens for this transaction.",
          );
        }
        if (error.message?.includes("Invalid recipient")) {
          throw new Error(
    ...
    onSuccess: (_data) => {
      console.log(
        "[useSendPuffin] Mutation onSuccess callback, invalidating queries",
      );
  ...
  return useQuery<bigint>({
    ...
    queryFn: async () => {
      ...
      try {
        ...
        console.log(
          "[useGetUnclaimedRewards] Success - Rewards:",
          rewards.toString(),
          "e8s",
        );
        return rewards;
        ...
        console.error(
          "[useGetUnclaimedRewards] Error fetching unclaimed rewards:",
          error,
        );
        throw error;
      }
  ...
  return useQuery<RewardHistory[]>({
    ...
    queryFn: async () => {
      ...
      try {
        ...
        console.log(
          "[useGetRewardHistory] Success - Found",
          history.length,
          "claims",
        );
        return history;
        ...
        console.error(
          "[useGetRewardHistory] Error fetching reward history:",
          error,
        );
        throw error;
      }
...
export function useClaimRewards() {
  ...
  return useMutation({
    mutationFn: async () => {
      ...
      try {
        ...
        if (error.message?.includes("Daily reward limit reached")) {
          throw new Error(
            "Daily reward limit reached. Please try again tomorrow.",
          );
        }
        if (error.message?.includes("timeout")) {
          throw new Error(
            "Claim timeout: Please check your connection and try again.",
          );
        }
    ...
    onSuccess: () => {
      console.log(
        "[useClaimRewards] Mutation onSuccess callback, invalidating queries",
      );
      queryClient.invalidateQueries({ queryKey: ["unclaimedRewards"] });
