...
export default function ProfileSetupModal() {
  ...
  const handleSave = async () => {
    ...
    try {
      ...
      toast.success('Welcome to PuffinHub! 🐧 Your profile has been created successfully!');
    } catch (error) {
    ...
    <Dialog open={true}>
      <DialogContent className="sm:max-w-[600px] max-h-[90vh] overflow-y-auto">
        ...
        <div className="relative z-10 space-y-6 py-4">
          ...
          {/* Avatar Selection */}
          <div className="space-y-2">
            <Label className="text-sm font-semibold">Choose Your NFT-Style Puffin Avatar</Label>
            <p className="text-xs text-muted-foreground">Select from our exclusive collection of collectible Puffin NFT art</p>
              ...
              {PRESET_AVATARS.map((avatar) => (
                ...
                >
                  ...
                  <div className="absolute bottom-0 left-0 right-0 bg-gradient-to-t from-black/60 to-transparent p-2">
                    <p className="text-[10px] font-semibold text-white text-center leading-tight">{avatar.label}</p>
                  </div>
                </button>
              ))}
            </div>
