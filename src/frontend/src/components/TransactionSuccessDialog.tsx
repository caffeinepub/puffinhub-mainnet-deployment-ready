...
export default function TransactionSuccessDialog({
  ...
  useEffect(() => {
    if (open && autoCloseDuration > 0) {
      console.log(
        "[TransactionSuccessDialog] Setting auto-close timer for",
        autoCloseDuration,
        "ms",
      );
  ...
  const handleOpenChange = (newOpen: boolean) => {
    console.log(
      "[TransactionSuccessDialog] Open state change requested:",
      newOpen,
    );
  ...
  return (
    <Dialog open={open} onOpenChange={handleOpenChange}>
      <DialogContent className="sm:max-w-md bg-white dark:bg-slate-900 border-2 border-primary/30">
        ...
        {/* Floating feathers */}
        <div
          className="absolute right-4 top-4 opacity-30 pointer-events-none animate-bounce"
        ...
        <div
          className="absolute left-6 bottom-6 opacity-20 pointer-events-none animate-bounce"
