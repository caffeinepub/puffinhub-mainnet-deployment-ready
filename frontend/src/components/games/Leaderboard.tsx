...
export default function Leaderboard() {
  ...
  const puffinTowerDefenseQuery = useGetLeaderboard(GameMode.puffinTowerDefense);
  const puffinColonyWarsQuery = useGetLeaderboard(GameMode.puffinColonyWars);

  const renderLeaderboardTable = (data: Array<{ playerName: string; score: bigint; principal?: any }> | undefined, isLoading: boolean) => {
    if (isLoading) {
      return (
    ...
    const sortedData = [...data].sort((a, b) => Number(b.score - a.score)).slice(0, 10);
          ...
          {sortedData.map((entry, index) => (
            <TableRow key={index}>
              ...
              <TableCell>
                {entry.principal ? (
                  <button
                    onClick={() => navigate({ to: `/profile/${entry.principal.toString()}` })}
              ...
              <TableCell className="text-right font-bold">{Number(entry.score)}</TableCell>
            </TableRow>
          ))}
        </TableBody>
      </Table>
  ...
  return (
    <Card>
      <CardHeader>
        ...
        <CardDescription>Top 10 players for each game mode - click names to view profiles</CardDescription>
      </CardHeader>
      <CardContent>
        <Tabs defaultValue="flightAdventure">
          ...
          <TabsContent value="flightAdventure" className="mt-4">
            {renderLeaderboardTable(flightAdventureQuery.data, flightAdventureQuery.isLoading)}
          </TabsContent>
          <TabsContent value="fishFrenzy" className="mt-4">
            {renderLeaderboardTable(fishFrenzyQuery.data, fishFrenzyQuery.isLoading)}
          </TabsContent>
          <TabsContent value="puffinRescue" className="mt-4">
            {renderLeaderboardTable(puffinRescueQuery.data, puffinRescueQuery.isLoading)}
          </TabsContent>
          <TabsContent value="arcticSurf" className="mt-4">
            {renderLeaderboardTable(arcticSurfQuery.data, arcticSurfQuery.isLoading)}
          </TabsContent>
          <TabsContent value="puffinCatch" className="mt-4">
            {renderLeaderboardTable(puffinCatchQuery.data, puffinCatchQuery.isLoading)}
          </TabsContent>
          <TabsContent value="puffinTowerDefense" className="mt-4">
            {renderLeaderboardTable(puffinTowerDefenseQuery.data, puffinTowerDefenseQuery.isLoading)}
          </TabsContent>
          <TabsContent value="puffinColonyWars" className="mt-4">
            {renderLeaderboardTable(puffinColonyWarsQuery.data, puffinColonyWarsQuery.isLoading)}
          </TabsContent>
        </Tabs>
      </CardContent>
    </Card>
