...
export default function PuffinColonyWarsGame({
  onBack,
}: PuffinColonyWarsGameProps) {
  ...
  useEffect(() => {
    ...
    const unitImages = [
      {
        key: "scout",
        src: "/assets/generated/puffin-scout-unit-transparent.dim_48x48.png",
      },
      {
        key: "warrior",
        src: "/assets/generated/puffin-warrior-unit-transparent.dim_48x48.png",
      },
      {
        key: "guard",
        src: "/assets/generated/puffin-guard-unit-transparent.dim_48x48.png",
      },
      {
        key: "knight",
        src: "/assets/generated/puffin-knight-unit-transparent.dim_48x48.png",
      },
      {
        key: "archer",
        src: "/assets/generated/puffin-archer-unit-transparent.dim_48x48.png",
      },
      {
        key: "tank",
        src: "/assets/generated/puffin-tank-unit-transparent.dim_48x48.png",
      },
      {
        key: "mech",
        src: "/assets/generated/puffin-mech-unit-transparent.dim_48x48.png",
      },
      {
        key: "sniper",
        src: "/assets/generated/puffin-sniper-unit-transparent.dim_48x48.png",
      },
      {
        key: "heavy",
        src: "/assets/generated/puffin-heavy-unit-transparent.dim_48x48.png",
      },
      {
        key: "cyber",
        src: "/assets/generated/puffin-cyber-unit-transparent.dim_48x48.png",
      },
      {
        key: "laser",
        src: "/assets/generated/puffin-laser-unit-transparent.dim_48x48.png",
      },
      {
        key: "titan",
        src: "/assets/generated/puffin-titan-unit-transparent.dim_48x48.png",
      },
    ];
    ...
    baseImg.src =
      "/assets/generated/puffin-colony-base-cropped-transparent.dim_96x96.png";
    baseSpriteRef.current = baseImg;

    // Load sniper sprite for all turret levels
    const turretImg = new Image();
    turretImg.src =
      "/assets/generated/puffin-sniper-unit-transparent.dim_48x48.png";
  ...
  const upgradeTurret = (base: Base) => {
    ...
    const nextLevelData = TURRET_LEVELS.find(
    ...
    if (nextLevelData && base.turret.experience >= nextLevelData.xpRequired) {
      ...
      const nextNextLevel = TURRET_LEVELS.find(
        (t) => t.level === currentLevel + 2,
      );
      if (nextNextLevel) {
  ...
  const handleBaseAttackUpgrade = () => {
    if (!isGameActive || resourcesRef.current < BASE_ATTACK_UPGRADE_COST)
      return;
  ...
  const initGame = () => {
    ...
    setAbilities([{ name: "Snowball Storm", cooldown: 0, maxCooldown: 30000 }]);
  ...
  const spawnUnit = (unitTypeIndex: number, isPlayer: boolean) => {
    ...
    const unit: Unit = {
      ...
      attackRange:
        unitType.range + (isPlayer ? globalRangeBonusRef.current : 0),
      isPlayer,
      type: unitType.name,
      era: unitType.era,
      cost: unitType.cost,
  ...
  const activateAbility = (abilityIndex: number) => {
    ...
    setAbilities((prev) => {
      ...
      newAbilities[abilityIndex] = {
        ...newAbilities[abilityIndex],
        cooldown: newAbilities[abilityIndex].maxCooldown,
      };
      return newAbilities;
    });
  };
  ...
  const drawTopHUD = (ctx: CanvasRenderingContext2D) => {
    ...
    // Resources
    ctx.fillText(
      `Resources: ${Math.floor(resourcesRef.current)}`,
      currentX,
      hudY,
    );
    currentX += spacing;
  ...
  const drawBase = (
    ...
    ctx.fillText(
      `${Math.max(0, Math.floor(base.health))}`,
      base.x,
      healthBarY - 4,
    );
  };
  ...
  const drawUnit = (ctx: CanvasRenderingContext2D, unit: Unit) => {
    const unitTypeData = UNIT_TYPES.find(
      (u) => u.name === unit.type && u.era === unit.era,
    );
  ...
  const gameLoop = () => {
    ...
    if (playerBaseRef.current.turretCooldown <= 0) {
      const nearestEnemy = enemyUnitsRef.current.find(
        (u) =>
          Math.abs(u.x - playerBaseRef.current.x) <
          playerBaseRef.current.turret.range,
      );
      if (nearestEnemy) {
    ...
    if (enemyBaseRef.current.turretCooldown <= 0) {
      const nearestPlayer = playerUnitsRef.current.find(
        (u) =>
          Math.abs(u.x - enemyBaseRef.current.x) <
          enemyBaseRef.current.turret.range,
      );
      if (nearestPlayer) {
      ...
      if (snowball.y >= snowball.targetY) {
        enemyUnitsRef.current.forEach((unit) => {
          const dist = Math.sqrt(
            (unit.x - snowball.x) ** 2 + (unit.y - snowball.y) ** 2,
          );
          if (dist < 40) {
    ...
    playerUnitsRef.current = playerUnitsRef.current.filter((unit) => {
      ...
      const friendlyAhead = playerUnitsRef.current.find(
        (u) => u !== unit && u.x > unit.x && u.x - unit.x < FORMATION_DISTANCE,
      );

      // Attack logic
      if (
        unit.targetLocked &&
        enemyUnitsRef.current.includes(unit.targetLocked)
      ) {
    ...
    enemyUnitsRef.current = enemyUnitsRef.current.filter((unit) => {
      ...
      // Check if there's a friendly unit ahead within formation distance
      const friendlyAhead = enemyUnitsRef.current.find(
        (u) => u !== unit && u.x < unit.x && unit.x - u.x < FORMATION_DISTANCE,
      );

      // Attack logic
      if (
        unit.targetLocked &&
        playerUnitsRef.current.includes(unit.targetLocked)
      ) {
        const distance = Math.abs(unit.x - unit.targetLocked.x);
        if (distance <= unit.attackRange) {
          if (unit.attackCooldown <= 0) {
    ...
    const spawnInterval = Math.max(
      3500,
      enemySpawnIntervalRef.current - eraRef.current * 250,
    );
    if (now - lastEnemySpawnRef.current > spawnInterval) {
      const enemyEra = Math.min(eraRef.current, 3);
      const possibleUnits = UNIT_TYPES.map((u, i) => ({
        ...u,
        index: i,
                ...
                <>
                  ...
                  <div className="absolute top-[70px] left-6 flex flex-col gap-2 z-30">
                    <Button
                      ...
                      disabled={
                        resources < getTurretUpgradeCost() ||
                        playerBaseRef.current.turret.level >= 4
                      }
                      size="sm"
                    ...
                    >
                      ...
                      <span className="font-bold text-[11px]">
                        Turret Lv.{playerBaseRef.current.turret.level}
                      </span>
                    ...
                    >
                      ...
                      <span className="text-[10px] font-semibold">
                        +{BASE_ATTACK_INCREASE}
                      </span>
                      <span className="text-[9px] font-semibold">
                        {BASE_ATTACK_UPGRADE_COST} 💰
                      </span>
                    </Button>
                    ...
                    <div className="bg-gradient-to-br from-blue-500/90 to-blue-600/90 backdrop-blur-md rounded-xl shadow-2xl border-2 border-white/30 p-3">
                      <div className="flex flex-wrap gap-2 justify-center items-center">
                        ...
                        {getAllUnits().map((unitType) => {
                          const unitIndex = UNIT_TYPES.findIndex(
                            (u) => u === unitType,
                          );
                          const isUnlocked = unitType.era <= era;
                          const canAfford =
                            resources >= unitType.cost && isUnlocked;
                          return (
                            <Button
                              key={unitIndex}
                              onClick={() =>
                                isUnlocked && handleSpawnUnit(unitIndex)
                              }
                              disabled={!canAfford}
                              size="sm"
                          ...
                          return (
                            ...
                            >
                              ...
                              <span className="font-bold text-[9px] leading-tight">
                                {isReady
                                  ? "Storm!"
                                  : `${Math.ceil(ability.cooldown / 1000)}s`}
                              </span>
                            </Button>
                          );
                        })}
              ...
              {showVictoryPopup && (
                <div className="absolute top-1/2 left-1/2 -translate-x-1/2 -translate-y-1/2 z-50 animate-in fade-in zoom-in duration-500">
                  <div className="bg-gradient-to-br from-green-500/95 to-emerald-600/95 backdrop-blur-md rounded-2xl shadow-2xl border-4 border-white/30 p-8 text-center min-w-[360px]">
                    ...
                    <p className="text-2xl font-semibold text-yellow-200 mb-2">
                      You Won!
                    </p>
                    <p className="text-lg text-white/90">
                      Enemy colony destroyed!
                    </p>
              ...
              {showRewardNotification && (
                <div className="absolute top-1/2 left-1/2 -translate-x-1/2 -translate-y-1/2 z-50 animate-in fade-in zoom-in duration-500">
                  <div className="bg-gradient-to-br from-amber-500/95 to-orange-600/95 backdrop-blur-md rounded-2xl shadow-2xl border-4 border-white/30 p-8 text-center min-w-[360px]">
                    ...
                    <p className="text-2xl font-semibold text-yellow-100 mb-2">
                      You earned 0.001 ICP
                    </p>
                  ...
                  return (
                    ...
                    >
                      ...
                      <span className="text-xs opacity-70">
                        HP:{unitType.health} DMG:{effectiveDamage} RNG:
                        {effectiveRange}
                      </span>
                    </Button>
                  );
              ...
              <div className="space-y-3">
                <div>
                  ...
                  <p className="text-sm text-muted-foreground mb-2">
                    Level: {playerBaseRef.current.turret.level} / 4 | Range:{" "}
                    {playerBaseRef.current.turret.range} | Damage:{" "}
                    {playerBaseRef.current.turret.damage}
                  </p>
                  <Button
                    onClick={handleManualTurretUpgrade}
                    disabled={
                  ...
                  >
                    {playerBaseRef.current.turret.level >= 4
                      ? "Max Level"
                      : `Upgrade Turret (${getTurretUpgradeCost()} 💰)`}
                  </Button>
                </div>
                ...
                <div className="pt-3 border-t">
                  ...
                  <p className="text-sm text-muted-foreground mb-2">
                    Max HP: {playerBaseRef.current.maxHealth} | +
                    {BASE_HP_INCREASE} HP per upgrade
                  </p>
                  <Button
                    onClick={handleBaseHPUpgrade}
                ...
                <div className="pt-3 border-t">
                  ...
                  <p className="text-sm text-muted-foreground mb-2">
                    Base Attack: {playerBaseRef.current.attack} | All Units: +
                    {globalAttackBonus} damage
                  </p>
                  ...
                  <Button
                    onClick={handleBaseAttackUpgrade}
                    disabled={
                      !isGameActive || resources < BASE_ATTACK_UPGRADE_COST
                    }
                    className="w-full"
                    size="sm"
                    variant="secondary"
                  >
                    Upgrade Attack (+{BASE_ATTACK_INCREASE} to all) (
                    {BASE_ATTACK_UPGRADE_COST} 💰)
                  </Button>
                </div>
                ...
                <div className="pt-3 border-t">
                  ...
                  <p className="text-sm text-muted-foreground mb-2">
                    Base Range: {playerBaseRef.current.attackRange} | All Units:
                    +{globalRangeBonus} range
                  </p>
                  <p className="text-xs text-muted-foreground mb-2 italic">
                  ...
                  <Button
                    onClick={handleBaseRangeUpgrade}
                    disabled={
                      !isGameActive || resources < BASE_RANGE_UPGRADE_COST
                    }
                    className="w-full"
                    size="sm"
                    variant="secondary"
                  >
                    Upgrade Range (+{BASE_RANGE_INCREASE} to all) (
                    {BASE_RANGE_UPGRADE_COST} 💰)
                  </Button>
                </div>
              </div>
              ...
              <div className="pt-4 border-t">
                ...
                {abilities.map((ability, index) => {
                  ...
                  const cooldownPercent =
                    (ability.cooldown / ability.maxCooldown) * 100;
        ...
        <DialogContent className="max-w-2xl max-h-[80vh] overflow-y-auto">
          <DialogHeader>
            <DialogTitle className="text-2xl">
              How to Play: Puffin Colony Wars
            </DialogTitle>
            <DialogDescription className="space-y-4 pt-4">
              <p className="text-base font-semibold text-foreground">
                Build your puffin army and destroy the enemy colony before they
                destroy yours!
              </p>
              <div className="space-y-3 text-base">
                <div className="bg-muted/50 p-3 rounded-lg">
                  ...
                  <p className="text-sm">
                    Destroy the enemy nest while defending your own!
                  </p>
                </div>
                <div className="bg-muted/50 p-3 rounded-lg">
                  <h4 className="font-semibold mb-1 flex items-center gap-2 text-foreground">
                  ...
                  <p className="text-sm">
                    Start with 25 resources and earn 2 per second automatically.
                    Gain more by defeating enemy units (half their cost). Your
                    base turret also earns resources when it kills enemies!
                  </p>
                </div>
                <div className="bg-muted/50 p-3 rounded-lg">
                  <h4 className="font-semibold mb-1 flex items-center gap-2 text-foreground">
                  ...
                  <p className="text-sm">
                    Units automatically march and form battle lines. Melee units
                    fight at the front, while ranged units attack from behind
                    their allies. When a front unit dies, the unit behind
                    advances to fill the gap.
                  </p>
                </div>
                <div className="bg-muted/50 p-3 rounded-lg">
                  <h4 className="font-semibold mb-1 flex items-center gap-2 text-foreground">
                  ...
                  <p className="text-sm">
                    Defeat enemies to gain experience. Reach XP thresholds to
                    advance through 4 eras, unlocking stronger puffin warriors!
                    Turret kills also contribute to your overall XP. Check the
                    HUD to see how much XP you need for the next era!
                  </p>
                </div>
                <div className="bg-muted/50 p-3 rounded-lg">
                  <h4 className="font-semibold mb-1 flex items-center gap-2 text-foreground">
                  ...
                  <p className="text-sm">
                    Your nest has turrets that automatically attack nearby
                    enemies and earn resources from kills. Manually upgrade
                    turrets for 200 (level 2), 500 (level 3), or 1000 (level 4)
                    resources. Each upgrade extends turret range and improves
                    damage! You can also upgrade your base HP (150 resources,
                    +300 HP), army attack (100 resources, +5 damage to ALL
                    units), and army range (120 resources, +15 range to ALL
                    units). Attack and range upgrades affect your entire army!
                  </p>
                </div>
                <div className="bg-muted/50 p-3 rounded-lg">
                  <h4 className="font-semibold mb-1 flex items-center gap-2 text-foreground">
                  ...
                  <p className="text-sm">
                    Free special ability with 30-second cooldown. Drops
                    snowballs that damage all enemies in the area!
                  </p>
                </div>
                <div className="bg-muted/50 p-3 rounded-lg">
                  <h4 className="font-semibold mb-1 flex items-center gap-2 text-foreground">
                  ...
                  <p className="text-sm">
                    Click unit buttons at the bottom of the screen to spawn
                    warriors. Use the upgrade buttons on the left to improve
                    your defenses and strengthen your entire army. Click the
                    special ability button when ready to unleash the Snowball
                    Storm!
                  </p>
                </div>
              </div>
