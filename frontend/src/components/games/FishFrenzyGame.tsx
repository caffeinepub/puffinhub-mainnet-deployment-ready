...
export default function FishFrenzyGame({ onBack }: FishFrenzyGameProps) {
  ...
  const puffinRef = useRef({ x: 400, y: 250, velocityX: 0, velocityY: 0, size: 40 });
  ...
  const initGame = () => {
    puffinRef.current = { x: 400, y: 250, velocityX: 0, velocityY: 0, size: INITIAL_PUFFIN_SIZE };
    fishesRef.current = [];
  ...
  const drawPuffin = (ctx: CanvasRenderingContext2D, puffin: { x: number; y: number; size: number }) => {
    ...
    ctx.beginPath();
    ctx.ellipse(puffin.x + 20 * scale, puffin.y + 25 * scale, 15 * scale, 20 * scale, 0, 0, Math.PI * 2);
    ctx.fill();

    // Belly
    ctx.fillStyle = '#F7F3E9';
    ctx.beginPath();
    ctx.ellipse(puffin.x + 20 * scale, puffin.y + 28 * scale, 10 * scale, 14 * scale, 0, 0, Math.PI * 2);
    ctx.fill();

    // Head
    ctx.fillStyle = '#2C3E50';
    ctx.beginPath();
    ctx.arc(puffin.x + 20 * scale, puffin.y + 12 * scale, 12 * scale, 0, Math.PI * 2);
    ctx.fill();

    // White eye dot
    ctx.fillStyle = '#FFFFFF';
    ctx.beginPath();
    ctx.arc(puffin.x + 23 * scale, puffin.y + 10 * scale, 2 * scale, 0, Math.PI * 2);
    ctx.fill();
  ...
  const gameLoop = () => {
    ...
    puffin.x = Math.max(0, Math.min(CANVAS_WIDTH - puffin.size, puffin.x + puffin.velocityX));
    puffin.y = Math.max(0, Math.min(CANVAS_HEIGHT - puffin.size, puffin.y + puffin.velocityY));
    ...
    fishesRef.current.forEach((fish) => {
      if (!fish.collected) {
        ...
        fishImg.src = '/assets/generated/fish-collectible-single-tooth-transparent.dim_64x64.png';
    ...
    killerFishesRef.current.forEach((killerFish) => {
      ...
      if (killerFish.y < 0 || killerFish.y > CANVAS_HEIGHT - killerFish.height) {
        killerFish.velocityY *= -1;
      }

      // Draw killer fish using new single-tooth sprite
      const killerFishImg = new Image();
      killerFishImg.src = '/assets/generated/killer-fish-single-tooth-transparent.dim_128x96.png';
      ctx.drawImage(killerFishImg, killerFish.x, killerFish.y, killerFish.width, killerFish.height);
      ...
      <div className="mx-auto max-w-4xl">
        <div className="mb-8 flex items-center gap-4">
          ...
          <div>
            ...
            <p className="mt-2 text-muted-foreground">
              Catch fish to grow bigger, but avoid the killer fish! Use arrow keys or WASD to swim.
            </p>
          </div>
        </div>
        ...
        <div className="mt-6 text-center space-y-4">
          {gameOver && showNameInput && (
            <Card>
              <CardHeader>
                <CardTitle>{defeatedByKiller ? 'Defeated by Killer Fish!' : 'Game Over!'}</CardTitle>
                <CardDescription>
                  {defeatedByKiller 
          ...
          <DialogHeader>
            ...
            <DialogDescription className="space-y-4 pt-4">
              <p className="text-base">
                Use <strong>arrow keys</strong> to swim. Catch fish to grow bigger, but avoid the killer fish! Catch as many fish as you can!
              </p>
              <ul className="list-disc list-inside space-y-2 text-base">
                <li>Use <strong>↑↓←→ (Arrow Keys)</strong> or <strong>WASD</strong> to swim in all directions</li>
                <li>Swim into <strong>small orange fish</strong> to catch them and earn <strong>10 points</strong> each</li>
                <li>Each fish you catch makes you <strong>grow bigger</strong> - there's no size limit!</li>
                <li><strong>Avoid the large red killer fish</strong> - they will end your game instantly!</li>
