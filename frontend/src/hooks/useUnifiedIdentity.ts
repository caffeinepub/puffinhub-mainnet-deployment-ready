...
const UnifiedIdentityReactContext = createContext<ProviderValue | undefined>(undefined);
...
function assertProviderPresent(context: ProviderValue | undefined): asserts context is ProviderValue {
    if (!context) {
        throw new Error('UnifiedIdentityProvider is not present. Wrap your component tree with it.');
    }
}
...
export function UnifiedIdentityProvider({
    ...
    const [currentProvider, setCurrentProvider] = useState<IdentityProvider | undefined>(undefined);
    ...
    const login = useCallback((provider: IdentityProvider) => {
        if (!authClient) {
            setErrorMessage(
    ...
    return createElement(UnifiedIdentityReactContext.Provider, { value, children });
}
