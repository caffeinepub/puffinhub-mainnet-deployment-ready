...
export class EditCommunicator {
  ...
  #selectedElements: Map<
    string,
    { id: string; element: HTMLElement; domTreeString: string }
  > = new Map();

  #boundMouseOverListener: (event: MouseEvent) => void =
    this.#handleMouseOver.bind(this);
  #boundMouseOutListener: (event: MouseEvent) => void =
  ...
  #rebuildSelection(ids: string[]): string[] {
    ...
    if (selectedElementIds.length > 0) {
      ...
      this.#toParent({
        type: "status",
        payload: { status: this.#editorState.type },
      });
    }
  ...
  constructor() {
    ...
    window.addEventListener(
      "message",
      (event: MessageEvent<ParentToEditCommunicatorMessage>) => {
        if (!allowedOrigins.includes(event.origin)) {
          this.#toParent({
            type: "error",
            payload: { msg: `disallowed origin: ${event.origin}` },
          });
          return;
        }
        ...
        switch (type) {
          ...
          case "clear-selection":
            this.#toParent({
              type: "status",
              payload: { status: "clearing selections" },
            });
            this.#clearAllSelections();
            break;
          case "remove-selection":
            if (payload?.element) {
              this.#toParent({
                type: "status",
                payload: { status: `removing ${payload.element}` },
              });
